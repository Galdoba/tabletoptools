...
func (dp *Dicepool) Label(label string) *Dicepool {
	dp.msg = label
	return dp
}
...
func (d *Dicepool) Select(things ...interface{}) (int, error) {
	if len(things) < 1 {
		return 0, fmt.Errorf("selection pool not provided")
	}
	if len(things) == 1 {
		return 0, nil
	}
	code := "1d" + fmt.Sprintf("%v", len(things))
	return d.Sroll(code) - 1, nil
}
