...
func RollPopulation(context profile.Profile, dice *dice.Dicepool) (string, error) {
	code := ""
	err := errors.New("roll was not made")
	switch context.Field(profile.GENERATION_METHOD) {
	case method.Basic:
		code, err = basicPopulationRoll(context, dice)
	}
	return code, err
}

func basicPopulationRoll(context profile.Profile, dice *dice.Dicepool) (string, error) {
	r := dice.Sroll("2d6-2")
	for r > 9 {
		r2 := dice.Sroll("1d6")
		if r2 < 6 {
